...
Once your project is ready, you should be able to run

```bash
node app.js --watch
```

Where app.js would be whatever you named your main entry point.

Your new project should have folders named "routes", "views", and a file called app.js

We'll setup routes for handling form data, using EJS for templating.

First, we'll install any necessary packages:

```bash
npm i ejs express express-validator http-errors morgan
```

Now modify your app.js file so it looks like this
...
Let's open up the routes folder, the index.js file and add a new simple route below any existing ones.
...
In the search bar, navigate to `localhost:(yourport)/`
...
(You might need to restart your server by pressing CTRL C and entering the watch command again) Now navigate to localhost/ (remember that our form is at our index, or home route).
...
In our router.post, we're going to add some methods to make sure we get the type of entries that we want.
...
If you tried filling out the form with only numbers, you should see a page containing JSON data containing our errors array. This is not very heplful to our users!

We should display our errors nicely on the page so the user knows what to correct.

The first thing we should change is our index.js router file, to properly pass down an errors object to our view.

This is a handy feature in express, where we can pass down what are called *locals* which are just data from our middleware that our view can then display.
...
router.get('/', function(req, res, next) {
  // Add in an empty errros array, otherwise we will get an undefined error when we first render our form
...
const errors = validationResult(req);
if (!errors.isEmpty()) {
// on our failure condition, pass down a local variable named errors which contains an array of errors
  return res.status(400).render('index', { title: 'Forms', errors: errors.array() })
}
...
The last thing we need to do is display our errors on the page in our template. Open up index.ejs, and add this *before* the form's html
...
This is a standard loop that should be familiar to you by now. Notice that nowhere in our file do we need to import, or require the error local variable; Express takes care of all of that for us.

Now, you should be able to enter some numbers, or a name that's too short or too long, and see the error messages displayed on the page. (You may need to restart your server for these changes) Once corrected, these errors should be gone, and the first and last name should be created.
